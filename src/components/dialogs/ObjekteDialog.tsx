...
interface ObjekteDialogProps {
  ...
  defaultValues?: Omit<Objekte['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['objekte']?.['status'] ?? [],
};
...
export function ObjekteDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ObjekteDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "titel": string | null, // Titel\n  "adresse": string | null, // Adresse\n  "status": LookupValue | null, // Status (select one key: "verfuegbar" | "reserviert" | "verkauft") mapping: verfuegbar=Verfügbar, reserviert=Reserviert, verkauft=Verkauft\n  "preis": number | null, // Preis (€)\n  "beschreibung": string | null, // Beschreibung\n  "verfuegbar_ab": string | null, // YYYY-MM-DD\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'adresse': (
      <div key="adresse" className="space-y-1.5">
        ...
        <Input
          id="adresse"
          placeholder="z. B. Straße 42, 10405 Berlin"
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'reserviert'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'reserviert' ? undefined : 'reserviert') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'reserviert'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'verkauft'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'verkauft' ? undefined : 'verkauft') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'verkauft'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Lage, Ausstattung, Besonderheiten..."
    ...
    'verfuegbar_ab': (
      <div key="verfuegbar_ab" className="space-y-1.5">
        <Label htmlFor="verfuegbar_ab">Verfügbar ab</Label>
        <DatePicker
          id="verfuegbar_ab"
          placeholder="Ab wann ist es frei?"
          mode="date"
          value={fields.verfuegbar_ab ?? null}
          onChange={v => setFields(f => ({ ...f, verfuegbar_ab: v ?? undefined }))}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "adresse": "Adresse", "status": "Status", "preis": "Preis (€)", "fotos": "Fotos", "beschreibung": "Beschreibung", "verfuegbar_ab": "Verfügbar ab"};
