...
export default function BesichtigungenPage() {
  ...
  const [records, setRecords] = useState<Besichtigungen[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Besichtigungen | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Besichtigungen | null>(null);
  ...
  async function handleCreate(fields: Besichtigungen['fields']) {
  ...
  async function handleUpdate(fields: Besichtigungen['fields']) {
    ...
    await LivingAppsService.updateBesichtigungenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteBesichtigungenEntry(deleteTarget.record_id);
  ...
  function getInteressentenDisplayName(url?: unknown) {
    if (!url) return '—';
    const id = extractRecordId(url);
    return interessentenList.find(r => r.record_id === id)?.fields.vorname ?? '—';
  }
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Besichtigungen im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('interessent')}>
                <span className="inline-flex items-center gap-1">
                  Interessent
                  {sortKey === 'interessent' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/besichtigungen/${record.record_id}`); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getInteressentenDisplayName(record.fields.interessent)}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Besichtigungen. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
